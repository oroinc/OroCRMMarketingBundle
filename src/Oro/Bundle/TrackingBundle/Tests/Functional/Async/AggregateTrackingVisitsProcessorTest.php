...
class AggregateTrackingVisitsProcessorTest extends WebTestCase
{
    ...
    {
        // Manually cleans up DB for MySQL because of the broken DB isolation caused by TRUNCATE TABLE statement in
        // {@see \Oro\Bundle\TrackingBundle\Migration\FillUniqueTrackingVisitsQuery}.
        ...
        if ($conn->getDatabasePlatform() instanceof MySqlPlatform) {
            foreach ([UniqueTrackingVisit::class, TrackingVisit::class, TrackingWebsite::class] as $class) {
                $managerRegistry
                    ->getRepository($class)
